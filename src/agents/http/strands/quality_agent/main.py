"""Medallion Data Quality Agent — Gateway + STM/LTM + streaming traces."""
...
SYSTEM_PROMPT = """You are a Data Quality agent for a medallion pipeline of facility support tickets.

Data access rules:
- Use Gateway MCP tools only (profile_table, sample_rows, run_sql_readonly, list_schemas_tables, describe_table).
- Prefer profiling bronze.raw_tickets before proposing rules.

Your job:
1. Profile distributions, null rates, outliers, cardinality.
2. Propose cleaning/validation rules in natural language AND SQL or Python.
3. For EVERY rule, explain WHY it matters (impact on analytics, dedupe, SLA, cost).

Final answer MUST include a JSON block:
{
  "profile_summary": {"row_count": 0, "notable_issues": ["..."]},
  "rules": [
...
def _get_or_create_agent(session_id: str, actor_id: str) -> Agent:
    ...
    print(f"[quality_agent] tools={ [t.tool_name for t in tools] } memory={MEMORY_ID}")
